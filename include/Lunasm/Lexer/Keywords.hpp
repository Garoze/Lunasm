...
static bool is_instruction(std::string_view text)
{
    return INSTRUCTIONS.find(text) != INSTRUCTIONS.end();
}

const std::unordered_map<std::string_view, Kind::kind_t> DIRECTIVES = {
    { "module", Kind::kind_t::Module },
    { "macro", Kind::kind_t::Macro },
    { "org", Kind::kind_t::Org },
};

static bool is_directive(std::string_view text)
{
    return DIRECTIVES.find(text) != DIRECTIVES.end();
}
